...
class ModeloPlanes
{
    ...
    static public function mdlMostrarPlanes($item, $valor)
    {
        if ($item != null) {
            ...
            try {
                $planes = Conexion::conectar()->prepare("SELECT p.id_plan, p.codigo, p.nombre, p.descripcion, p.duracion, p.cantidad_sesiones, p.id_entrenador, p.estado, e.nombre, e.apellido FROM planes p INNER JOIN entrenadores e ON p.id_entrenador = e.id_entrenador");
                $planes->execute();
