...
class ModeloEntrenadores
{
    ...
    static public function mdlMostrarEntrenadores($item, $valor)
    {
        if ($item != null) {
            ...
            try {
                $entrenadores = Conexion::conectar()->prepare("SELECT e.id_entrenador, e.nombre, e.apellido, e.dni, e.telefono, e.email, e.id_especialidad, e.fecha_contratacion, e.estado FROM entrenadores e INNER JOIN especialidades es ON e.id_entrenador = es.id_entrenador");
